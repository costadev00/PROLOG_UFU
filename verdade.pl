...
% primo(X,Y) :- homem(X), (pai(Z,X),irmao(Z,A),filho(Y,A)) ;(mae(Z,X),irma(Z,A), filho(Y,A)).
primo(X,Y) :- homem(X),mae(Z,X), mae(A,Y), irma(A,Z); homem(X), pai(Z,X), pai(A,Y), irmao(A,Z).
primo(X,Y) :- mulher(X),mae(Z,X), mae(A,Y), irma(A,Z).
